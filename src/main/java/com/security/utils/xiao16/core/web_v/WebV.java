package com.security.utils.xiao16.core.web_v;
...
//////
   ...
   /* public static HttpResponse requestX(String url, byte[] submit) {
        HttpResponse res;
        if (submit != null) {
            res = HttpRequest.post(url).body(submit).execute();
        } else {
    ...
    public static HttpResponse requestX(String url, byte[] submit, String header) {
        HttpResponse res;
        if (submit != null) {
            res = HttpRequest.post(url).body(submit).addHeaders(createHeaders(header)).execute();
        } else {
    ...
    public static HttpResponse requestX(String url, byte[] submit, String header, List<HttpCookie> cookies) {
        HttpResponse res;
        if (submit != null) {
            res = HttpRequest.post(url).body(submit).addHeaders(createHeaders(header)).cookie(cookies).execute();
        } else {
    ...
    public static HttpResponse requestX(String url, byte[] submit, String header, List<HttpCookie> cookies, String proxy) {
        HttpResponse res ;
        try {
            if (submit != null) {
                res = HttpRequest.post(url).body(submit).addHeaders(createHeaders(header)).cookie(cookies).setProxy(creatProxy(proxy)).execute();
